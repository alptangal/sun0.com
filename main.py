...
async def on_ready():
    ...
    try:
        req = requests.get("http://1localhost:8888")
        ...
        for thread in RESULT["threadsCh"].threads:
            ...
            THREADS.append(
                {"username": msgs[0].content.split("@")[1], "lastMsg": msgs[-1]}
            )
        if not like.is_running():
            like.start(guild)
        ...
        for user in usernames:
            ...
            for thread in RESULT["threadsCh"].threads:
                if email == thread.name:
                    isset = True
            if not isset:
                rs = await RESULT["threadsCh"].create_thread(
                    name=user.split("||")[1].replace("``", ""),
                    content="https://suno.com/@"
                    + user.split("||")[0].replace("``", "").lower(),
                )
                req = requests.get(
                    "https://suno.com/@" + user.split("||")[0].replace("``", "")
                )
                ...
                els = (
                    soup.find(
                        "div",
                        {
                            "class": "flex flex-col lg:flex-row gap-8 px-4 pb-8 lg:p-8 pt-16 items-center -mt-32 md:mt-0 z-[20]"
                        },
                    )
                    .find(
                        "div",
                        {"class": "flex flex-col lg:flex-row gap-8 w-full lg:w-auto"},
                    )
                    .find("div", {"class": "flex flex-col"})
                    .find("div", {"class": "flex flex-row gap-2"})
                    .find_all("div")
                )
                await rs.thread.send(
                    content=f"``LIKE``== **{els[0].getText().split(' ')[0]}** | ``FANS``== **{els[1].getText().split(' ')[0]}** | ``PLAYS``== **{els[2].getText().split(' ')[0]}**"
                )
        for thread in RESULT["threadsCh"].threads:
            try:
                msgs = [msg async for msg in thread.history(oldest_first=True)]
